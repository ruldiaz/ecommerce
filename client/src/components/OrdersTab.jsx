...
import { Box, TableContainer, Th, Tr, Table, Td, Thead, Tbody, Button, useDisclosure, Alert, Stack, Spinner, AlertIcon, AlertTitle, AlertDescription, Wrap, useToast, Text, Flex } from '@chakra-ui/react';
                ...
                {orders && orders.map((order)=> (
                  <Tr key={order._id}>
                    ...
                    <Td>{order.orderItems.map((item)=>(
                      <Text key={item._id}>{item.qty} x {item.name}</Text> // added index key
                    ))}</Td>
